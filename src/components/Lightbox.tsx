      ...
      >
        ...
        <div className="h-20 px-6 flex items-center justify-between z-30 bg-gradient-to-b from-neutral-950/80 to-transparent w-full">
          ...
          <div className="font-mono text-[10px] tracking-widest text-neutral-400">
            <span className="text-white font-serif italic text-sm mr-4">{photo.title}</span>
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              onClick={onClose}
              className="p-2.5 rounded-none bg-neutral-900 border border-neutral-800 text-neutral-400 hover:text-white transition-all duration-300 hover:scale-105 active:scale-95"
              data-cursor="nav"
            >
          ...
          <AnimatePresence>
            {showExif && photo.exif && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 30 }}
                onClick={(e) => e.stopPropagation()}
                className="absolute bottom-6 mx-auto bg-neutral-900/90 border border-neutral-800 backdrop-blur-md px-8 py-5 rounded-none flex flex-wrap gap-x-12 gap-y-3 justify-center items-center font-mono text-[10px] tracking-widest text-neutral-400 z-40 max-w-[90%] select-text"
              >
                <div className="flex flex-col items-center sm:items-start">
                  <span className="text-[8px] text-neutral-500 uppercase">Camera System</span>
                  <span className="text-white mt-0.5">{photo.exif.camera}</span>
                </div>
                <div className="w-px h-6 bg-neutral-800 hidden sm:block" />
                <div className="flex flex-col items-center sm:items-start">
                  <span className="text-[8px] text-neutral-500 uppercase">Optics Lens</span>
                  <span className="text-white mt-0.5">{photo.exif.lens}</span>
                </div>
                <div className="w-px h-6 bg-neutral-800 hidden sm:block" />
                <div className="flex flex-col items-center sm:items-start">
                  <span className="text-[8px] text-neutral-500 uppercase">Focal / Aperture</span>
                  <span className="text-white mt-0.5">{photo.exif.focalLength} @ {photo.exif.aperture}</span>
                </div>
                <div className="w-px h-6 bg-neutral-800 hidden sm:block" />
                <div className="flex flex-col items-center sm:items-start">
                  <span className="text-[8px] text-neutral-500 uppercase">Shutter / ISO</span>
                  <span className="text-white mt-0.5">{photo.exif.shutterSpeed} // ISO {photo.exif.iso}</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="h-16 px-6 flex items-center justify-between font-mono text-[9px] text-neutral-500 z-30 bg-gradient-to-t from-neutral-950/80 to-transparent w-full select-text">
